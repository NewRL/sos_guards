...
class sos_voucher(osv.osv):
	...
	_track = {
		'state': {
			'sos_voucher.mt_voucher_state_change': lambda self, cr, uid, obj, ctx=None: True,
		},
	}
...
#			res[voucher.id] = any([((line.account_id.type, 'in', ('receivable', 'payable')) and line.reconcile_id) for line in voucher.move_ids])
	...
	def _get_journal(self, cr, uid, context=None):
		if context is None: context = {}
		invoice_pool = self.pool.get('sos.invoice')
		journal_pool = self.pool.get('account.journal')
	...
	def _get_post(self, cr, uid, context=None):
		if context is None: context = {}
		return context.get('post_id', False)
	...
	def onchange_line_ids(self, cr, uid, ids, line_dr_ids, line_cr_ids, amount, voucher_currency, type, context=None):
		...
		for voucher_line in line_dr_ids+line_cr_ids:
			line_id = voucher_line.get('id') and self.pool.get('sos.voucher.line').browse(cr, uid, voucher_line['id'], context=context).move_line_id.id or voucher_line.get('move_line_id')
	...
	_columns = {
		...
		'date':fields.date('Date', readonly=True, states={'draft':[('readonly',False)]}, help="Effective date for accounting entries", copy=False),
		'journal_id':fields.many2one('account.journal', 'Journal', required=True, readonly=True, states={'draft':[('readonly',False)]}),
		'account_id':fields.many2one('account.account', 'Account', required=True, readonly=True, states={'draft':[('readonly',False)]}),
		'line_ids':fields.one2many('sos.voucher.line', 'voucher_id', 'Voucher Lines', readonly=True, copy=True, states={'draft':[('readonly',False)]}),
		'line_cr_ids':fields.one2many('sos.voucher.line','voucher_id','Credits', domain=[('type','=','cr')], context={'default_type':'cr'}, readonly=True, states={'draft':[('readonly',False)]}),
		'line_dr_ids':fields.one2many('sos.voucher.line','voucher_id','Debits',	domain=[('type','=','dr')], context={'default_type':'dr'}, readonly=True, states={'draft':[('readonly',False)]}),
		...
		'reference': fields.char('Ref #', readonly=True, states={'draft':[('readonly',False)]}, help="Transaction reference number.", copy=False),
		'number': fields.char('Number', readonly=True, copy=False),
		'move_id':fields.many2one('account.move', 'Account Entry', copy=False),
		'move_ids': fields.related('move_id','line_ids', type='one2many', relation='account.move.line', string='Journal Items', readonly=True),
		'partner_id':fields.many2one('res.partner', 'Partner', change_default=1, readonly=True, states={'draft':[('readonly',False)]}),
		'post_id':fields.many2one('sos.post', 'Partner', change_default=1, readonly=True, states={'draft':[('readonly',False)]}),
		...
		'pay_now':fields.selection([('pay_now','Pay Directly'),	('pay_later','Pay Later or Group Funds'),],'Payment', readonly=True, states={'draft':[('readonly',False)]}),
		...
		'payment_rate': fields.float('Exchange Rate', digits=(12,6), required=True, readonly=True, states={'draft': [('readonly', False)]},	help='The specific rate that will be used, in this voucher, between the selected currency (in \'Payment Rate Currency\' field)  and the voucher currency.'),
	...
	_defaults = {
		...
		'company_id': lambda self,cr,uid,c: self.pool.get('res.company')._company_default_get(cr, uid, 'sos.voucher',context=c),
		'tax_id': _get_tax,
	...
	def onchange_journal_multi_post(self, cr, uid, ids, journal_id, context=None):
		vals = {'value': {}}
		if not journal_id:
			return vals
		journal_pool = self.pool.get('account.journal')
		...
		currency_id = journal.company_id.currency_id.id or False
		if journal.currency:
	...
	def onchange_date_multi_post(self, cr, uid, ids, journal_id, date, context=None):
		period_pool = self.pool.get('account.period')
		res = self.onchange_journal_multi_post(cr, uid, ids, journal_id, context=context)
	...
	def compute_tax(self, cr, uid, ids, context=None):
		...
		post_pool = self.pool.get('sos.post')
		position_pool = self.pool.get('account.fiscal.position')
		voucher_line_pool = self.pool.get('sos.voucher.line')
		voucher_pool = self.pool.get('sos.voucher')
		if context is None: context = {}
		...
		for voucher in voucher_pool.browse(cr, uid, ids, context=context):
			...
			post = post_pool.browse(cr, uid, voucher.post_id.id, context=context) or False
			taxes = position_pool.map_tax(cr, uid, post and post.property_account_position or False, tax)
	...
	def onchange_price(self, cr, uid, ids, line_ids, tax_id, post_id=False, context=None):
		context = context or {}
		tax_pool = self.pool.get('account.tax')
		post_pool = self.pool.get('sos.post')
		position_pool = self.pool.get('account.fiscal.position')
		...
		for line in line_ids:
			...
			if tax_id:
				...
				if post_id:
					post = post_pool.browse(cr, uid, post_id, context=context) or False
					taxes = position_pool.map_tax(cr, uid, post and post.property_account_position or False, tax)
					tax = tax_pool.browse(cr, uid, taxes, context=context)
	...
	def onchange_journal_voucher(self, cr, uid, ids, line_ids=False, tax_id=False, price=0.0, post_id=False, journal_id=False, ttype=False, company_id=False, context=None):
		"""price
		...
		if not post_id or not journal_id:
			return default
		...
		journal = journal_pool.browse(cr, uid, journal_id, context=context)
		post = post_pool.browse(cr, uid, post_id, context=context)
		account_id = False
		tr_type = False
		if journal.type in ('sale','sale_refund'):
			account_id = post.property_account_receivable.id
			tr_type = 'sale'
		elif journal.type in ('purchase', 'purchase_refund','expense'):
			account_id = post.property_account_payable.id
			tr_type = 'purchase'
		else:
		...
		vals = self.onchange_journal(cr, uid, ids, journal_id, line_ids, tax_id, post_id, time.strftime('%Y-%m-%d'), price, ttype, company_id, context)
	...
	def onchange_amount(self, cr, uid, ids, amount, rate, post_id, journal_id, currency_id, ttype, date, payment_rate_currency_id, company_id, context=None):
		if context is None:
			context = {}
		ctx = context.copy()
		...
		res = self.recompute_voucher_lines(cr, uid, ids, post_id, journal_id, amount, currency_id, ttype, date, context=ctx)
	...
	def basic_onchange_post(self, cr, uid, ids, post_id, journal_id, ttype, context=None):
		post_pool = self.pool.get('sos.post')
		journal_pool = self.pool.get('account.journal')
	...
	def onchange_post_id(self, cr, uid, ids, post_id, journal_id, amount, currency_id, ttype, date, context=None):
		if not journal_id:
			return {}
		if context is None:
			context = {}
		...
		res = self.basic_onchange_post(cr, uid, ids, post_id, journal_id, ttype, context=context)
		if ttype in ['sale', 'purchase']:
			return res
		ctx = context.copy()
		# not passing the payment_rate currency and the payment_rate in the context but it's ok because they are reset in recompute_payment_rate
		ctx.update({'date': date})
		vals = self.recompute_voucher_lines(cr, uid, ids, post_id, journal_id, amount, currency_id, ttype, date, context=ctx)
		...
		# We should definitively split sos.voucher object in two and make distinct on_change functions. In the 
	...
	def recompute_voucher_lines(self, cr, uid, ids, post_id, journal_id, price, currency_id, ttype, date, context=None):
		"""
		Returns a dict that contains new values and context
		...
		post_pool = self.pool.get('sos.post')
		journal_pool = self.pool.get('account.journal')
		line_pool = self.pool.get('sos.voucher.line')
		...
		# These twi lines     or  next For loop
		#if line_ids:
...
#			ids = move_line_pool.search(cr, uid, [('state','=','valid'), ('account_id.type', '=', account_type), ('reconcile_id', '=', False), ('post_id', '=', post_id)], context=context)
#		else:
#			ids = context['move_line_ids']
		sos_invoice_id = context.get('invoice_id', False)
		...
		for line in account_move_lines:
			...
			if sos_invoice_id:
				if line.sos_invoice.id == sos_invoice_id:
		...
		for line in account_move_lines:
			...
			rs = {
				...
				'invoice_id': line.sos_invoice_id and line.sos_invoice_id.id or False,
				'post_id': post_id,
			}
			...
			if len(default['value']['line_cr_ids']) > 0:   # and ttype == 'payment'
				default['value']['pre_line'] = 1
			elif len(default['value']['line_dr_ids']) > 0:  # and ttype == 'receipt'
				default['value']['pre_line'] = 1
	...
	def onchange_journal(self, cr, uid, ids, journal_id, line_ids, tax_id, post_id, date, amount, ttype, company_id, context=None):
		if context is None:
			context = {}
		if not journal_id:
			return False
		...
		if context.get('payment_expected_currency') and currency_id != context.get('payment_expected_currency'):
			...
			amount = 0
		if post_id:
			res = self.onchange_post_id(cr, uid, ids, post_id, journal_id, amount, currency_id, ttype, date, context)
			for key in res.keys():
				vals[key].update(res[key])
		return vals

	def onchange_company(self, cr, uid, ids, post_id, journal_id, currency_id, company_id, context=None):
		"""
	...
	def proforma_voucher(self, cr, uid, ids, context=None):
		...
		self.update_invoice_status(cr, uid, ids, context=context)
		return True
	...
	def update_invoice_status(self, cr, uid, ids, context=None):
		voucher_pool = self.pool.get('sos.voucher')
		invoice_pool = self.pool.get('sos.invoice')
		...
		for voucher in self.browse(cr, uid, ids, context=context):
			...
			for line in voucher.line_ids:
				...
				if line.invoice_id.reconciled:
					invoice_pool.write(cr,uid,line.invoice_id.id,{'state':'paid'})
		return True
...
#		return True
	...
	def unlink(self, cr, uid, ids, context=None):
		...
		return super(sos_voucher, self).unlink(cr, uid, ids, context=context)

	def onchange_payment(self, cr, uid, ids, pay_now, journal_id, post_id, ttype='sale'):
		res = {}
		if not partner_id:
			return res
		res = {}
		...
		if pay_now == 'pay_later':
			post = post_pool.browse(cr, uid, post_id)
	...
	def _sel_context(self, cr, uid, voucher_id, context=None):
		...
		if current_currency <> company_currency:
			context_multi_currency = context.copy()
			voucher = self.pool.get('sos.voucher').browse(cr, uid, voucher_id, context)
	...
	def first_move_line_get(self, cr, uid, voucher_id, move_id, company_currency, current_currency, context=None):
		...
		move_line = {
				...
				'post_id': not(voucher.multi_partner) and voucher.post_id.id or False,
				'currency_id': company_currency <> current_currency and  current_currency or False,
				'amount_currency': (sign * abs(voucher.amount) # amount < 0 for refunds
					if company_currency != current_currency else 0.0),
				'date': voucher.date,
				'date_maturity': voucher.date_due,
				'project_id': not(voucher.multi_partner) and voucher.post_id.project_id.id or False,
				'center_id': not(voucher.multi_partner) and voucher.post_id.center_id.id or False,
			}
	...
	def _get_exchange_lines(self, cr, uid, line, move_id, amount_residual, company_currency, current_currency, context=None):
		...
		move_line = {
			...
			'account_id': line.account_id.id,
			'move_id': move_id,
			'post_id': line.voucher_id.post_id.id,
	...
	def check_amount_total(self, cr, uid, ids, context=None):
		...
		for voucher in self.browse(cr, uid, ids,context):
			...
			if float_compare(amount, voucher.amount, prec) and voucher.multi_partner or amount == 0.0:
	...
	def voucher_move_line_create(self, cr, uid, voucher_id, line_total, move_id, company_currency, current_currency, context=None):
		...
		ctx = context.copy()
		ctx.update({'date': date})
		voucher = self.pool.get('sos.voucher').browse(cr, uid, voucher_id, context=ctx)
		voucher_currency_id = voucher.currency_id and voucher.currency_id.id or voucher.journal_id.company_id.currency_id.id
		voucher_currency = currency_obj.browse(cr, uid, voucher_currency_id, context=ctx)
		ctx.update({
		...
		for line in voucher.line_ids:
			...
			move_line = {
				...
				'name': line.name or '/',
				'account_id': line.account_id.id,
				#'related_account_id': voucher.account_id.id,
				'move_id': move_id,
				'post_id': line.post_id.id or voucher.post_id.id,
				...
				'debit': 0.0,
				'date': voucher.date,
				'project_id': line.post_id.project_id.id or voucher.post_id.project_id.id,
				'center_id': line.post_id.center_id.id or voucher.post_id.center_id.id,
				'sos_invoice_id': line.invoice_id and line.invoice_id.id or False,
			}
			if amount < 0:
				amount = -amount
				if line.type == 'dr':
			...
			if line.move_line_id:
				...
				if line.move_line_id.currency_id and line.move_line_id.currency_id.id != company_currency:
					...
					if line.move_line_id.currency_id.id == current_currency:
					...
					elif line.move_line_id.currency_id.id == voucher.payment_rate_currency_id.id:
						...
						amount_currency = (move_line['debit'] - move_line['credit']) * voucher.payment_rate * voucher_rate
					else:
	...
	def _get_company_currency(self, cr, uid, voucher_id, context=None):
		...
		return self.pool.get('sos.voucher').browse(cr,uid,voucher_id,context).journal_id.company_id.currency_id.id
...
class sos_voucher_line(osv.osv):
	_name = 'sos.voucher.line'
	...
	_columns = {
		'voucher_id':fields.many2one('sos.voucher', 'Voucher', required=1, ondelete='cascade'),
		...
		'invoice_id' : fields.many2one('sos.invoice','Invoice'),
	...
	def default_get(self, cr, user, fields_list, context=None):
		...
		journal_pool = self.pool.get('account.journal')
		post_pool = self.pool.get('sos.post')
		values = super(sos_voucher_line, self).default_get(cr, user, fields_list, context=context)
		...
		if journal.type in ('sale', 'sale_refund'):
			...
			post = partner_pool.browse(cr, user, post_id, context=context)
...
#	def onchange_post_id(self, cr, uid, ids, post_id, journal_id, price, currency_id, ttype, invoice_id, context=None):
#		currency_pool = self.pool.get('res.currency')
#		move_line_pool = self.pool.get('account.move.line')
#		journal_pool = self.pool.get('account.journal')
#		invoice_pool = self.pool.get('sos.invoice')
...
#		ids = move_line_pool.search(cr, uid, [('state','=','valid'), ('account_id.type', '=', account_type), ('reconcile_id', '=', False), ('post_id', '=', post_id), ('move_id', '=', invoice_brw.move_id.id)], context=context)
...
#				'reconcile': (amount_original == amount_unreconciled) or False,
#				'invoice_id': invoice_id or (line.sos_invoice_id and line.sos_invoice_id.id) or False,
#			})
#			
#		return default
sos_voucher_line()

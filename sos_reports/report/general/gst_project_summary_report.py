...
class ReportGSTProjectSummary(models.AbstractModel):
	_name = 'report.sos_reports.report_gst_summaryproject'
	...
	def render_html(self, data=None):
		...
		for project in projects:
			...
			self.env.cr.execute("select sum(aml.debit) as amount from account_payment ap, account_move_line aml ,account_invoice ai, account_invoice_payment_rel rl where ap.id = aml.payment_id and aml.account_id = 147 \
				and aml.journal_id = 32 and aml.date >= %s and aml.date <= %s and ap.id = rl.payment_id and ai.id = rl.invoice_id and ai.project_id = %s",(date_from,date_to,project.id))

...
class hr_attendance(models.Model):
	...
	state = fields.Selection((('draft', 'Unverified'), ('verified', 'Verified'), ('locked', 'Locked'),('unlocked', 'Un-Locked')), 'State', 
		...
		for punch in self:
			if punch.state in ['verified', 'locked']:
				raise UserError(_("""You may not delete a record that is in a %s state:\n
					Employee: %s, Checkin: %s, Checkout: %s""")% (punch.state, punch.employee_id.name, punch.check_in,punch.check_out))
	...
	def write(self,vals):
		if (self.state in ['verified', 'locked'] and (vals.get('employee_id'))):
			raise UserError(_("""You may not write to a record that is in a %s state:\n
				Employee: %s, Checkin: %s, Checkout: %s""")% (self.state, self.employee_id.name, self.check_in,self.check_out))
